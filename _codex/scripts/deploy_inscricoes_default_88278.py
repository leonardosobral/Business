"""Single-file, baseline-guarded release. Reuses the existing reversible publisher.
...
receipt_name=sys.argv[2] if len(sys.argv)>2 else '2026-09-16_inscricoes_default_88278.json'
assert Path(receipt_name).name==receipt_name and receipt_name.endswith('.json')
receipt=root/'_codex/docs'/receipt_name
order=[('Business','inscricoes/includes/backend.cfm')]
baseline=['8fc09bc18ebf48a122faf2e06a11987f222dda20f1f9520725153a4e6a22af5f']
...
publisher=publisher.replace('rr-live-measurement','business-inscricoes-default-88278').replace('exactly 11 rows','exactly 1 row').replace('11 whitelisted runtime targets','1 whitelisted runtime target').replace('existing=9, absent=2','existing=1, absent=0')
...
if mode=='prepare':
 ...
 init=f'''
release=Path('/var/backups')/('business-inscricoes-default-88278.'+secrets.token_hex(6));release.mkdir(mode=0o700)
...
'''
 ...
 assert re.fullmatch(r'/var/backups/business-inscricoes-default-88278\.[A-Za-z0-9_]+',remote)
...
if {mode!r}=='publish':
 ...
 assert re.search(r'successful\\s+1\\b',log) and re.search(r'total\\s+1\\b',log),'Native compilation was not verified'
...
'''
if mode=='prepare':
 # Compile only the single CFML candidate, outside all webroots.
 remote_script+='''
stage=Path(tempfile.mkdtemp(prefix='inscricoes-default-88278-compile-',dir='/tmp'))
...
assert compiled_run.returncode==0 and re.search(r'successful\\s+1\\b',log) and re.search(r'total\\s+1\\b',log),log[-4000:]
'''
if mode=='publish':
