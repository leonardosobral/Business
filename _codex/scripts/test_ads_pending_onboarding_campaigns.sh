...
require_pattern "$form" 'Salvar como rascunho' "formulario permite salvar rascunho explicitamente"
require_pattern "$form" 'name="campaign_intent" value="submit"' "formulario permite salvar e enviar para analise"
...
require_pattern "$backend" 'qAdsV1AdminOperationalCampaigns' "admin possui consulta global de campanhas em operacao"
require_pattern "$backend" '(?s)qAdsV1AdminOperationalCampaigns.*?review\.status[[:space:]]*=[[:space:]]*\x27APPROVED\x27.*?campaign\.status[[:space:]]+IN[[:space:]]*\([^)]*\x27ACTIVE\x27[^)]*\x27PAUSED\x27' "visao operacional mostra somente campanhas aprovadas ativas ou pausadas"
...
require_pattern "$admin_home" 'Campanhas em operação' "admin mostra campanhas aprovadas em operacao"
require_pattern "$admin_home" 'qAdsV1AdminOperationalCampaigns' "visao operacional usa a consulta global administrativa"
require_pattern "$admin_home" 'Impressões|Visualizações' "visao operacional mostra alcance"
require_pattern "$admin_home" 'Cliques' "visao operacional mostra cliques"
