...
receipt_name=sys.argv[2] if len(sys.argv)>2 else '2026-09-14_event_interest_release_v2.json'
...
order=[('Business','portal/audiencia/queries/event_interest.sql'),
       ('Business','portal/audiencia/queries/event_agenda.sql'),
       ('Business','portal/includes/event_interest_backend.cfm'),
       ('Business','portal/eventos-analytics/home.cfm')]
baseline=['ABSENT','ABSENT','ABSENT','6cbf8e276af64f09e9c7bad27551f6f1c725565a54734f6d8b3875e29745f260']
...
publisher=base[:start]+'ORDER = '+repr(tuple(order))+'\nNEW_FILES = frozenset(ORDER[:3])\n'+base[end:]
publisher=publisher.replace('rr-live-measurement','business-event-interest').replace('exactly 11 rows','exactly 4 rows').replace('11 whitelisted runtime targets','4 whitelisted runtime targets').replace('existing=9, absent=2','existing=1, absent=3')
