"""Two-file, baseline-guarded release for the registrations event selector.

Reuses the existing reversible publisher. No database connections, migrations,
credentials, git mutations or service restarts.
"""
...
receipt_name = (
    sys.argv[2]
    if len(sys.argv) > 2
    else "2026-09-16_inscricoes_event_selector.json"
)
assert Path(receipt_name).name == receipt_name and receipt_name.endswith(".json")
receipt = root / "_codex/docs" / receipt_name
order = [
    ("Business", "inscricoes/includes/backend.cfm"),
    ("Business", "inscricoes/home.cfm"),
]
baseline = [
    "6bccdbc0e01ee2abcde4c1a209a2efc6ece3b4f9a2562f7559d9baa8f805e063",
    "2d4110b31fd6af561d81ba54b3674ae6234bffdda0244f5876c000fbb0fc49b5",
]
ssh = [
    "ssh",
    "-o",
...
start = base.index("ORDER = (")
end = base.index("GUARD_FILES =", start)
publisher = (
    base[:start]
    + "ORDER = "
    + repr(tuple(order))
...
publisher = (
    publisher.replace("rr-live-measurement", "business-inscricoes-event-selector")
    .replace("exactly 11 rows", "exactly 2 rows")
    .replace("11 whitelisted runtime targets", "2 whitelisted runtime targets")
    .replace("existing=9, absent=2", "existing=2, absent=0")
)
publisher = publisher.replace(
    '"logout.cfm",',
    '"logout.cfm", '
    '"includes/backend/backend_login.cfm", '
    '"includes/backend/business_account_context.cfm", '
    '"includes/backend/business_permissions.cfm", '
    '"inscricoes/index.cfm", '
    '"inscricoes/includes/form_campanha.cfm",',
)
...
publisher = publisher.replace(
    "if (site, relative) in NEW_FILES:", "if (site, relative) in ORDER:"
)
...
if mode == "prepare":
    ...
    init = f"""
release=Path('/var/backups')/('business-inscricoes-event-selector.'+secrets.token_hex(6));release.mkdir(mode=0o700)
...
"""
    ...
    remote = previous["directory"]
    assert re.fullmatch(
        r"/var/backups/business-inscricoes-event-selector\.[A-Za-z0-9_]+", remote
    )
...
if {mode!r}=='publish':
 ...
 assert re.search(r'successful\\s+2\\b',log) and re.search(r'total\\s+2\\b',log),'Native compilation was not verified'
...
if mode == "prepare":
    # Compile only the two CFML candidates, outside all webroots.
    remote_script += """
stage=Path(tempfile.mkdtemp(prefix='inscricoes-event-selector-compile-',dir='/tmp'))
...
assert compiled_run.returncode==0 and re.search(r'successful\\s+2\\b',log) and re.search(r'total\\s+2\\b',log),log[-4000:]
"""
if mode == "publish":
    # The immutable package is rechecked before installation. HTTP/UI checks are
    # a separate required post-publication step before verify is accepted as done.
    pass
remote_script += f"""
...
response = subprocess.run(
    ssh + ["python3 -c " + shlex.quote(remote_script)],
    input=payload,
    capture_output=True,
    timeout=220,
)
if response.stdout:
    ...
    destination = (
        receipt
        if mode == "prepare"
        else receipt.with_name(receipt.stem + "-" + mode + ".json")
    )
...
if response.stderr:
    print(response.stderr.decode(), file=sys.stderr)
