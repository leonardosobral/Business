"""Nine-file, baseline-guarded release. Reuses the existing reversible publisher.
...
receipt_name=sys.argv[2] if len(sys.argv)>2 else '2026-09-14_result_import_groups_release.json'
assert Path(receipt_name).name==receipt_name and receipt_name.endswith('.json')
receipt=root/'_codex/docs'/receipt_name
order=[('Business','services/queries/.htaccess'),
       ('Business','services/queries/result_import_context.sql'),
       ('Business','services/queries/result_import_archive.sql'),
       ('Business','services/ResultImportQueueService.cfc'),
       ('Business','racetag/includes/backend.cfm'),
       ('Business','racetag/form.cfm'),
       ('Business','assets/js/result-import-queue.js'),
       ('Business','administracao/importacoes-resultados/includes/backend.cfm'),
       ('Business','administracao/importacoes-resultados/home.cfm')]
baseline=['ABSENT','ABSENT','ABSENT','ABSENT',
 'ac584ff57b345375ad1af26a660e1f5d260e48b237a42a403257a003d2b05aee',
 '256fcbe3df53cebdcff55bdc675cf5d0615ed582906f5f78b4b11f7ba3f86eea',
 '151e73378d30a4a89e1570b947e90e370df283cc1305f1cbbdf47ecf2d789f2e',
 '7225687c5df41aa1d8b4f8fb4a463829d0f4e5228e70a7a88b14fe59d50aae49',
 '72985d4d4550dddb02e49e6fdd19aff99a1380417663c14061e255dc114b8db8']
...
publisher=base[:start]+'ORDER = '+repr(tuple(order))+'\nNEW_FILES = frozenset(ORDER[:4])\n'+base[end:]
publisher=publisher.replace('rr-live-measurement','business-result-import-groups').replace('exactly 11 rows','exactly 9 rows').replace('11 whitelisted runtime targets','9 whitelisted runtime targets').replace('existing=9, absent=2','existing=5, absent=4')
...
if mode=='prepare':
 ...
 init=f'''
queries=Path('/var/www/business.roadrunners.run/services/queries')
assert not queries.is_symlink()
queries.mkdir(mode=0o755,exist_ok=True)
release=Path('/var/backups')/('business-result-import-groups.'+secrets.token_hex(6));release.mkdir(mode=0o700)
...
'''
 ...
 assert re.fullmatch(r'/var/backups/business-result-import-groups\.[A-Za-z0-9_]+',remote)
...
if {mode!r}=='publish':
 ...
 assert re.search(r'successful\\s+5\\b',log) and re.search(r'total\\s+5\\b',log),'Native compilation was not verified'
...
'''
if mode=='prepare':
 # Compile only the five CFML candidates, outside all webroots.
 remote_script+='''
stage=Path(tempfile.mkdtemp(prefix='result-import-groups-compile-',dir='/tmp'))
...
assert compiled_run.returncode==0 and re.search(r'successful\\s+5\\b',log) and re.search(r'total\\s+5\\b',log),log[-4000:]
'''
if mode=='publish':
