"""Two-file, baseline-guarded release. Reuses the existing reversible publisher.
...
receipt_name=sys.argv[2] if len(sys.argv)>2 else '2026-09-17_event_description_batch.json'
assert Path(receipt_name).name==receipt_name and receipt_name.endswith('.json')
receipt=root/'_codex/docs'/receipt_name
order=[('Business','services/EventDescriptionRewriteService.cfc'), ('Business','api/eventos/jobs/rewrite-descriptions.cfm')]
baseline=['bee90db7d8cfe72657f7023b7fe04ef4acd35ea958aa233fe5d27c12c2468e21','cb6adff0daff8c2dcc3efa73183b7835687200502f02c4050e8a64942a434e77']
...
publisher=publisher.replace('rr-live-measurement','business-description-batch').replace('exactly 11 rows','exactly 2 rows').replace('11 whitelisted runtime targets','2 whitelisted runtime targets').replace('existing=9, absent=2','existing=2, absent=0')
publisher=publisher.replace('"logout.cfm",','"logout.cfm", "inscricoes/home.cfm", "inscricoes/index.cfm", "includes/backend/backend_login.cfm", "administracao/importacoes-resultados/includes/backend.cfm", "services/ResultImportQueueService.cfc", "racetag/form.cfm", "racetag/includes/backend.cfm", "assets/js/result-import-queue.js", "includes/backend/require_admin.cfm", "portal/eventos-analytics/index.cfm", "portal/includes/event_analytics_backend.cfm",')
...
if mode=='prepare':
 ...
 init=f'''
release=Path('/var/backups')/('business-description-batch.'+secrets.token_hex(6));release.mkdir(mode=0o700)
...
'''
 ...
 assert re.fullmatch(r'/var/backups/business-description-batch\.[A-Za-z0-9_]+',remote)
...
if mode=='prepare':
 ...
 remote_script+='''
stage=Path(tempfile.mkdtemp(prefix='description-batch-compile-',dir='/tmp'))
