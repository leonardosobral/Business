...
const scratch=mkdtempSync(resolve(tmpdir(),'audience-capacity-backend-'));
try {
  ...
  writeFileSync(backendPath,backend.replace(/\bqueryExecute\(/g,'capacityFixtureQuery('));
  copyFileSync(resolve(root,'_codex/tests/audience-capacity/backend_fixture.cfm'),resolve(scratch,'check.cfm'));
  ...
  assert.ok(result.stdout.includes('CAPACITY_BACKEND_PASS_8'),`Backend contract failed: ${result.stdout.slice(-6000)}`);
  console.log('Capacity backend: 8 behavior assertions passed (isolated database boundary).');
