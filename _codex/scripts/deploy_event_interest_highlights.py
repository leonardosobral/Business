...
receipt_name=sys.argv[2] if len(sys.argv)>2 else '2026-09-14_event_interest_highlights_release_v2.json'
assert Path(receipt_name).name==receipt_name and receipt_name.endswith('.json')
receipt=root/'_codex/docs'/receipt_name
order=[('Business','portal/audiencia/queries/event_interest.sql'),
       ('Business','portal/includes/event_interest_backend.cfm'),
       ('Business','portal/eventos-analytics/home.cfm')]
baseline=['81063c2c6621bfa91c118fac0ba5c842a1c1c802d1ca1cbbd3c64dc503c7566e','03e3874d205c00f81ab1757d5acf0154d2743cf8277e5c7d8027e42179f1c884','7f775b363f36631df90814f8ec2e86fae473a6bfbd1ea083527de12515373b5a']
...
publisher=publisher.replace('rr-live-measurement','business-event-interest').replace('exactly 11 rows','exactly 3 rows').replace('11 whitelisted runtime targets','3 whitelisted runtime targets').replace('existing=9, absent=2','existing=3, absent=0')
...
if mode=='prepare':
 ...
 init=f'''
release=Path('/var/backups')/('business-event-interest.'+secrets.token_hex(6));release.mkdir(mode=0o700)
...
'''
 ...
 assert re.fullmatch(r'/var/backups/business-event-interest\.[A-Za-z0-9_]+',remote)
...
'''
if mode=='prepare':
 # Compile only the two new CFML candidates, outside all webroots.
 remote_script+='''
