"""Six-file, baseline-guarded release. Reuses the existing reversible publisher.
...
receipt_name=sys.argv[2] if len(sys.argv)>2 else '2026-09-17_audience_origin_campaign.json'
assert Path(receipt_name).name==receipt_name and receipt_name.endswith('.json')
receipt=root/'_codex/docs'/receipt_name
order=[('Business','portal/audiencia/queries/origins.sql'),
...
baseline=['ABSENT','10be889aee67cb298032c75bb43e79c5d48132f8810337d79acc7018e44a924a','4411c8297b86965029a9c69ba9e38fc2ed575df26be7abdda5fd61146a61eb08','34ce8e517875667f35fb5c1631ff800f9540f0202c7eab21a2611ac24cb0ff5e','1aa489f4bb73a9764f078e826ae08991e4986185e65709783bf124bca179ca6c','69ef25fc1f0db3ae35edc43818db6e21222f090129e3433820c63e283ea8c358']
...
publisher=publisher.replace('rr-live-measurement','business-origin-campaign').replace('exactly 11 rows','exactly 6 rows').replace('11 whitelisted runtime targets','6 whitelisted runtime targets').replace('existing=9, absent=2','existing=5, absent=1')
...
if mode=='prepare':
 ...
 init=f'''
release=Path('/var/backups')/('business-origin-campaign.'+secrets.token_hex(6));release.mkdir(mode=0o700)
...
'''
 ...
 assert re.fullmatch(r'/var/backups/business-origin-campaign\.[A-Za-z0-9_]+',remote)
...
'''
if mode=='prepare':
 # Compile only the four CFML candidates, outside all webroots.
 remote_script+='''
stage=Path(tempfile.mkdtemp(prefix='origin-campaign-compile-',dir='/tmp'))
