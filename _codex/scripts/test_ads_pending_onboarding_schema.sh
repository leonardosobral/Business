...
require_pattern "$schema" 'GRANT[[:space:]]+SELECT[[:space:]]+ON[[:space:]]+ads\.campaign_review_history' "historico permite leitura operacional"
reject_pattern "$schema" 'GRANT[^;]*INSERT[^;]*campaign_review_history' "historico aceita append somente pelas funcoes controladas"
