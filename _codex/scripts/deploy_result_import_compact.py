...
receipt_name=sys.argv[2] if len(sys.argv)>2 else '2026-09-14_result_import_compact_release.json'
assert Path(receipt_name).name==receipt_name and receipt_name.endswith('.json')
receipt=root/'_codex/docs'/receipt_name
order=[('Business','administracao/importacoes-resultados/home.cfm')]
baseline=['322ee60469c860b7eb08a31801980e237bb4b902dcc55028a6bebd062a6628d8']
...
publisher=publisher.replace('rr-live-measurement','business-result-import-compact').replace('exactly 11 rows','exactly 1 row').replace('11 whitelisted runtime targets','1 whitelisted runtime target').replace('existing=9, absent=2','existing=1, absent=0')
publisher=publisher.replace('"logout.cfm",','"logout.cfm", "administracao/importacoes-resultados/includes/backend.cfm", "services/ResultImportQueueService.cfc", "racetag/form.cfm", "racetag/includes/backend.cfm", "assets/js/result-import-queue.js", "includes/backend/require_admin.cfm", "portal/eventos-analytics/index.cfm", "portal/includes/event_analytics_backend.cfm",')
...
if mode=='prepare':
 ...
 init=f'''
release=Path('/var/backups')/('business-result-import-compact.'+secrets.token_hex(6));release.mkdir(mode=0o700)
...
'''
 ...
 assert re.fullmatch(r'/var/backups/business-result-import-compact\.[A-Za-z0-9_]+',remote)
...
if mode=='prepare':
 ...
 remote_script+='''
stage=Path(tempfile.mkdtemp(prefix='result-import-compact-compile-',dir='/tmp'))
