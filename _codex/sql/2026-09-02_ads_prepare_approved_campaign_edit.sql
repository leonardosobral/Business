-- Reabre uma campanha aprovada como rascunho para edicao.
-- A campanha para de veicular imediatamente e exige uma nova revisao.
...
BEGIN
    ...
    IF campaign.status NOT IN ('ACTIVE', 'PAUSED') THEN
        RAISE EXCEPTION 'Somente campanha ACTIVE ou PAUSED pode iniciar nova edicao';
    END IF;
    ...
    IF NOT FOUND OR review.status <> 'APPROVED' THEN
        RAISE EXCEPTION 'Somente campanha aprovada pode ser reaberta para edicao';
    END IF;
...
VALUES (
    '2026-09-02_ads_prepare_approved_campaign_edit',
    'Pausa campanha aprovada e a reabre como rascunho para nova revisao'
)
