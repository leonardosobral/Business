...
assert.match(page, /Reprocessar falhas recuperáveis/);
...
assert.match(endpoint, /provider_temporarily_unavailable/);
assert.match(endpoint, /statuscode=503/);
...
assert.match(service, /provider_cooldown/);
assert.match(service, /provider_temporarily_unavailable/);
assert.match(service, /attempts=GREATEST\(attempts-1,0\)/);
assert.match(service, /WHERE status='pending'/);
assert.match(service, /isRetryableProviderError/);
assert.match(service, /NOT LIKE '%fonte não tem texto suficiente%'/);
...
assert.match(provider, /assertSuccessfulResponse/);
assert.match(provider, /ArticleSummary\.OpenAI\.Retryable/);
assert.match(provider, /rate_limit_exceeded|errorCode/);
assert.match(config, /promptVersion = "4"/);
assert.match(config, /providerCooldownMinutes/);
