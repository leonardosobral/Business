...
import {
  ChartRenderer, compact, DataComponent, DataTable, Filters, MetricCard,
  percentage, periodComparison, previousPeriodRows, RichNarrative, SectionHeader, SortableItem, SortableRegion, useDataApp, useSectionFilters,
...
function ScenarioLever({ label, value, min, max, onCommit }) {
  ...
  return <label className="scenario-lever"
    style={{ "--slider-progress": `${(liveValue - min) / (max - min) * 100}%` }}>
    <span>{label}</span>
    <output>{percentage(Math.round(liveValue) / 100)}</output>
    <input type="range" min={min} max={max} step={1} value={liveValue} onChange={updateValue} />
  </label>;
}
...
export function DashboardContent() {
  ...
  return <>
      ...
      {visible("usage-trend") && <SortableItem id="usage-trend" label="Active accounts over time" kind="chart" span={12} minSpan={3}>
        <DataComponent variant="card" padding="spacious" id="usage-trend" title="Active accounts over time" queryId="usage_summary"
          kind="chart" chart={trend} className="feature-chart dashboard-panel">
          <ChartRenderer spec={trend} rows={trendRows}
            height={260} {...chartProps("usage-trend")} />
        </DataComponent>
        </SortableItem>}
        ...
        <DataComponent variant="card" id="growth-drivers"
          title="Weekly change in active accounts" queryId="growth_drivers" kind="chart" chart={drivers}
          ...
          <ChartRenderer spec={{ ...drivers, ...(previousSummary?.activeUsers != null
            && summary?.activeUsers != null
            ? { beginning: previousSummary.activeUsers, ending: summary.activeUsers } : {}) }}
