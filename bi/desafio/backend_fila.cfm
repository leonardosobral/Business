...
<cfquery name="qStatsEvento">
    ...
    inner join public.desafios dc on usr.id = dc.id_usuario
    ...
    AND dc.desafio IN ('todosantodia','desafiofoco')
    group by act.athlete_id, act.athlete_id,
    usr.id, usr.strava_code, usr.strava_id, to_timestamp(usr.strava_expires_at), usr.estado, usr.name, pag.tag
