            ...
            <cfif URL.periodo NEQ "pendentes" AND NOT VARIABLES.challengeIsCircuitPanel>
                <td class="text-start" nowrap>
                    <cfif isDefined("strava_code") AND len(trim(qStatsEvento.strava_code))>
                        <a target="_blank" href="https://roadrunners.run/api/strava/atualizar/?desafio=#URL.desafio#&id_usuario=#id#&token=#urlEncodedFormat(structKeyExists(APPLICATION, 'eventoApiToken') ? APPLICATION.eventoApiToken : '')#&debug=true"><div class="badge bg-strava"><i class="fa fa-refresh"></i></div></a>
                        <a target="_blank" href="https://roadrunners.run/api/strava/atualizar/?desafio=#URL.desafio#&id_usuario=#id#&token=#urlEncodedFormat(structKeyExists(APPLICATION, 'eventoApiToken') ? APPLICATION.eventoApiToken : '')#&debug=true&full=true"><div class="badge bg-black me-1"><i class="fa fa-refresh"></i></div></a>
                        <a target="_blank" href="https://roadrunners.run/api/strava/atualizar/?desafio=#URL.desafio#&id_usuario=#id#&token=#urlEncodedFormat(structKeyExists(APPLICATION, 'eventoApiToken') ? APPLICATION.eventoApiToken : '')#&debug=true&full=promax"><div class="badge <cfif len(trim(data_statisticas))>bg-secondary<cfelse>bg-black</cfif> me-1"><i class="fa fa-mobile"></i></div></a>
