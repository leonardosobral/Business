    ...
    <table id="tblEventos" class="table table-stripped table-condensed table-sm mb-0">
        ...
        <tr style="font-size: small;">
            ...
            <td class="text-start">
                ...
                <cfif isDefined("strava_expires_at") AND len(trim(strava_expires_at))> <cfif DateDiff("n",strava_expires_at, now()) LT 0>#DateDiff("n",strava_expires_at, now()) * -1# min válido<cfelse><span class="text-danger">token expirado</span></cfif></cfif></cfif>
            </td>
        </tr>
        </cfoutput>
        </tbody>
