(function attachMifPortfolio(globalScope) {
  ...
  function renderSummary(root, summary) {
    ...
    root.innerHTML = [
      ...
      section(SUMMARY_SECTIONS[3], 'Portfólio · 4', 'Dependências observadas', 'As células preservam fase, distância e estado; concentração não é previsão de substituição.', `${dependencySampleNote}${report.renderBarChart({ title: 'Top 10 células de concentração observada', description: 'Inscrições selecionadas por célula publicada.', rows: dependencies, labelKey: 'label', valueKey: 'value', denominator: overview.paid_registrations, source: 'Células phase × modality × state; exposição usa total do evento e total comercial.' })}${report.renderTable({ columns: [{ key: 'label', label: 'Célula' }, { key: 'channel_name', label: 'Canal', format: report.formatChannelName }, { key: 'paid_registrations', label: 'Inscrições', format: report.formatInteger }, { key: 'sample_status', label: 'Qualificação da amostra' }, { key: 'exposure', label: 'Cobertura em risco' }], rows: dependencyTop })}`),
