(function attachMifPortfolio(globalScope) {
  ...
  const MIN_VISIBLE_CELL_REGISTRATIONS = 5;
  ...
  function classifyExposure(selected, eventTotal, commercialTotal, minimumProfileRegistrations) {
    if (selected < MIN_VISIBLE_CELL_REGISTRATIONS || eventTotal <= 0) return null;
    const eventShare = selected / eventTotal * 100;
    const commercialShare = commercialTotal ? selected / commercialTotal * 100 : 0;
    if (selected >= minimumProfileRegistrations && eventShare >= 40) return 'alta';
    if (selected >= minimumProfileRegistrations && eventShare >= 20) return 'média';
    if (commercialShare >= 60 && eventShare < 20) return 'dependência entre parceiros';
    return 'baixa';
  }
  ...
  function simulate(data, selection) {
    ...
    for (const row of data?.coverage_cube || []) {
      ...
      if (channelName === ALL_CHANNELS) cell.event_paid_registrations += registrations;
      else if (channelName === COMMERCIAL_CHANNELS) cell.commercial_paid_registrations += registrations;
    ...
    const minimumProfileRegistrations = toNumber(data?.thresholds?.minimum_profile_registrations) || 10;
    ...
    const publishable = stableRows(
      completeCells.filter((cell) => cell.selected_paid_registrations >= MIN_VISIBLE_CELL_REGISTRATIONS),
      [
    ...
    const suppressed = completeCells.filter((cell) => cell.selected_paid_registrations < MIN_VISIBLE_CELL_REGISTRATIONS);
    const selectedPaidRegistrations = completeCells.reduce((total, cell) => total + cell.selected_paid_registrations, 0);
    const eventPaidRegistrations = completeCells.reduce((total, cell) => total + cell.event_paid_registrations, 0);
    const commercialPaidRegistrations = completeCells.reduce((total, cell) => total + cell.commercial_paid_registrations, 0);
  ...
  function topTen(rows, labelKey, valueKey) {
    const report = requireReport();
    return report.topNWithOthers(rows || [], labelKey, valueKey, 10);
  }
  ...
  function dimensionPanel(dimension, panel) {
    ...
    const nearestPeers = Object.entries(panel?.nearest_peers || {}).map(([channel_name, peer]) => ({
      channel_name,
  ...
  function renderSummary(root, summary) {
    ...
    const redundancy = (summary?.redundancy_candidates || []).map((row) => ({
      ...
      value: toNumber(row.qualifying_dimension_count),
    }));
    ...
    const redundancyTop = topTen(redundancy, 'pair', 'value');
    const dependencyTop = topTen(dependencies, 'label', 'value');
    ...
    root.innerHTML = [
      ...
      section(SUMMARY_SECTIONS[2], 'Portfólio · 3', 'Redundância observada', 'Pares semelhantes em dimensões qualificadas merecem leitura conjunta, sem score mestre.', `${report.renderBarChart({ title: 'Top 10 pares com redundância observada', description: 'Quantidade de dimensões qualificadas por par.', rows: redundancy, labelKey: 'pair', labelFormatter: report.formatChannelName, valueKey: 'value', denominator: redundancy.length, source: 'Semelhanças dimensionais com cobertura mínima explicitada.' })}${report.renderTable({ columns: [{ key: 'pair', label: 'Par', format: report.formatChannelName }, { key: 'qualifying_dimensions', label: 'Dimensões qualificadas' }, { key: 'combined_gross_value', label: 'Valor bruto combinado', format: report.formatCurrency }], rows: redundancyTop })}`),
