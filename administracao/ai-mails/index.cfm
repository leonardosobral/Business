...
<link rel="stylesheet" href="/administracao/ai-mails/assets/ai-mails.css?v=3"/>
...
<main class="container-fluid px-3 px-lg-4 business-page admin-suite-page admin-suite-main ai-mails" id="aiMails" data-csrf="<cfoutput>#encodeForHtmlAttribute(session.aiMailCsrf)#</cfoutput>">
    ...
    <section class="mail-workspace" aria-label="Conversas analisadas">
        <nav class="mail-views" aria-label="Visões de atendimento"><button type="button" data-view="pending" aria-pressed="true">Pendentes</button><button type="button" data-view="in_progress" aria-pressed="false">Em andamento</button><button type="button" data-view="resolved" aria-pressed="false">Resolvidos</button><button type="button" data-view="review" aria-pressed="false">Revisão da triagem</button><button type="button" data-view="batches" aria-pressed="false"><i class="fa-solid fa-layer-group me-1" aria-hidden="true"></i>Lotes <span id="mailBatchOpenCount" class="mail-view-count">0</span></button></nav>
        ...
        <div id="mailBatchBar" class="mail-batch-bar">
            <label class="mail-check mail-check-compact"><input type="checkbox" id="mailSelectPage"/> Selecionar esta página</label>
            <span id="mailSelectedCount">Nenhuma conversa selecionada</span>
            <div class="mail-actions"><button id="mailClearSelection" type="button" class="btn btn-outline-light" disabled>Limpar</button><button id="mailAnalyzeBatch" type="button" class="btn btn-warning" disabled><i class="fa-solid fa-wand-magic-sparkles me-2" aria-hidden="true"></i>Analisar como lote</button></div>
        </div>
    ...
    <dialog id="mailBatchCreate" class="mail-dialog" aria-labelledby="mailBatchCreateTitle"><form id="mailBatchCreateForm"><div class="mail-dialog-top"><div><span class="admin-suite-kicker">Análise consolidada</span><h2 class="h4 mb-0" id="mailBatchCreateTitle">Criar lote de conversas</h2></div><button type="button" class="btn btn-outline-light" data-close>Fechar</button></div>
        <p id="mailBatchCreateCount" class="mail-notice"></p>
        <p>O AI-mails vai comparar os resumos já existentes, identificar padrão ou causa comum, separar duplicidades e sugerir um plano único. Nenhuma ação será executada automaticamente.</p>
        <label>Título do lote <span class="mail-muted">(opcional)</span><input class="form-control mt-1" name="title" maxlength="200" placeholder="Ex.: Falhas recorrentes no Push"/></label>
        <label>O que você quer entender? <span class="mail-muted">(opcional)</span><textarea class="form-control mt-1" name="instruction" maxlength="1200" rows="4" placeholder="Ex.: Verifique se os erros parecem ter a mesma origem e indique a ordem de correção."></textarea></label>
        <p class="mail-muted">Os corpos dos e-mails não são reenviados nesta etapa: a consolidação usa somente os resumos já analisados e continua restrita aos admins globais.</p>
        <p id="mailBatchCreateStatus" class="text-warning" role="status"></p>
        <div class="mail-actions"><button type="submit" class="btn btn-warning"><i class="fa-solid fa-wand-magic-sparkles me-2" aria-hidden="true"></i>Analisar e criar lote</button><button type="button" class="btn btn-outline-light" data-close>Cancelar</button></div>
    </form></dialog>
    <dialog id="mailBatchDetail" class="mail-dialog mail-detail mail-batch-detail" aria-labelledby="mailBatchDetailTitle"><div class="mail-dialog-top"><span class="admin-suite-kicker">Tratamento em lote</span><button type="button" class="btn btn-outline-light" data-close>Fechar</button></div><div id="mailBatchDetailBody"></div></dialog>
    <dialog id="mailConfig" class="mail-dialog" aria-labelledby="mailConfigTitle"><form id="mailConfigForm"><div class="mail-dialog-top"><h2 class="h4" id="mailConfigTitle">Configurações do AI-mails</h2><button type="button" class="btn btn-outline-light" data-close>Fechar</button></div>
        ...
        <div class="mail-config-grid"><label>Modelo OpenAI<input class="form-control" name="model" value="gpt-4.1-mini" maxlength="100" required/></label><label>Limite de análises por dia<input class="form-control" name="daily_limit" type="number" min="1" max="2000" value="100" required/></label><label>Retenção de resolvidos (dias)<input class="form-control" name="retention_days" type="number" min="7" max="365" value="90" required/></label></div>
        <p class="mail-muted">O monitor considera somente novas mensagens recebidas depois do corte informado no painel. Mensagens anteriores não entram na fila. Anexos não são analisados.</p>
...
<script src="/administracao/ai-mails/assets/ai-mails.js?v=6"></script>
</body></html>
