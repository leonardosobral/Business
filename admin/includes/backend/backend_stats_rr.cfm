...
<cfquery name="qPeriodo" dbtype="query">
    ...
    <cfif len(trim(URL.preset)) AND URL.preset EQ "28dias">
        where log_timestamp >= <cfqueryparam cfsqltype="cf_sql_timestamp" value="#now()-28#"/>
    </cfif>
</cfquery>
...
<cfquery name="qCount28" cachedwithin="#CreateTimeSpan(0, 1, 0, 0)#">
    select count(*) as total
    from tb_log log
    where log.log_item = 'evento'
    and log.site = 'RR'
    AND log_timestamp >= <cfqueryparam cfsqltype="cf_sql_timestamp" value="#now()-28#"/>
    <cfif URL.preset EQ "bot">
        and (log_user_agent ilike '%bot%' OR  log_user_agent ilike '%crawler%')
    <cfelse>
        and log_user_agent NOT ilike '%bot%' and log_user_agent NOT ilike '%crawler%'
