<cfquery name="qBase" cachedwithin="#CreateTimeSpan(0, 0, 1, 0)#">
    SELECT COALESCE(uf.regiao, 'Não Informada') as regiao, usr.*, pag.tag as tag_usuario,
...
<cfquery name="qCountHoje" dbtype="query">
    ...
    from qBase
    where data_criacao >= <cfqueryparam cfsqltype="cf_sql_timestamp" value="#now()-1#"/>
</cfquery>

<cfquery name="qCount7" dbtype="query">
    select count(*) as total
    from qBase
    where data_criacao >= <cfqueryparam cfsqltype="cf_sql_timestamp" value="#now()-7#"/>
</cfquery>

<cfquery name="qCount28" dbtype="query">
    select count(*) as total
    from qBase
    where data_criacao >= <cfqueryparam cfsqltype="cf_sql_timestamp" value="#now()-28#"/>
</cfquery>
